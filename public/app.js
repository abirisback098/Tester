...
function setAuthView(isSignedIn){$('planner-app').classList.toggle('hidden',!isSignedIn)}
function revealApp(){document.body.classList.remove('auth-boot')}
function render(){renderDays();const day=ensureLocalDay(selectedDay),blocks=day.blocks;$('day-title').textContent=DAYS[selectedDay];$('day-subtitle').textContent=blocks.length?`${blocks.length} ${blocks.length===1?'time block':'time blocks'} · Changes save automatically.`:'Empty routine. Start at 6:00 AM.';$('blocks').innerHTML=blocks.map(b=>`<tr data-id="${b.id}"><td><input class="time-input" type="time" value="${minsToTime(b.start_minutes)}" data-field="start_minutes" aria-label="Start time"></td><td><input class="duration-input" type="text" inputmode="numeric" value="${esc(durationText(b.duration_minutes))}" data-field="duration" aria-label="Duration" placeholder="1h 30m"></td><td><input class="cell-input" value="${esc(b.activity)}" data-field="activity" aria-label="Activity" placeholder="What are you doing?"></td><td><textarea class="cell-textarea" data-field="notes" aria-label="Notes" placeholder="Optional notes">${esc(b.notes)}</textarea></td><td><div class="reminder-wrap"><input class="reminder-toggle" type="checkbox" ${b.notification_enabled?'checked':''} data-field="notification_enabled" aria-label="Enable reminder"><select class="reminder-select" data-field="notification_offset_minutes" ${b.notification_enabled?'':'disabled'} aria-label="Reminder time"><option value="0" ${b.notification_offset_minutes===0?'selected':''}>At start</option><option value="5" ${b.notification_offset_minutes===5?'selected':''}>5 min before</option><option value="10" ${b.notification_offset_minutes===10?'selected':''}>10 min before</option><option value="15" ${b.notification_offset_minutes===15?'selected':''}>15 min before</option><option value="30" ${b.notification_offset_minutes===30?'selected':''}>30 min before</option></select></div></td><td><button class="delete-button" data-action="delete" aria-label="Delete block">×</button></td></tr>`).join('');$('empty-state').style.display=blocks.length?'none':'flex';$('add-block').disabled=!session;$('empty-add').disabled=!session;$('copy-day').disabled=!session||!blocks.length;scheduleNotifications()}
...
async function saveBlock(block){if(!session||!supabase)return;const day=ensureLocalDay(selectedDay);let routineId=day.id;if(!routineId){const {data,error}=await supabase.from('routines').insert({user_id:session.user.id,day_of_week:selectedDay,title:''}).select('id').single();if(error){toast(error.message);return}routineId=day.id=data.id}const payload={routine_id:routineId,position:block.position,start_minutes:block.start_minutes,duration_minutes:block.duration_minutes,activity:block.activity,notes:block.notes,notification_enabled:block.notification_enabled,notification_offset_minutes:block.notification_offset_minutes};const {data,error}=await supabase.from('blocks').upsert({...payload,id:block.id},{onConflict:'id'}).select('id').single();if(error){toast(error.message);return}block.id=data.id;block._dirty=false;$('save-state').textContent='Saved'}
...
async function mutateBlock(id,field,value){const blocks=currentBlocks(),i=blocks.findIndex(b=>b.id===id);if(i<0)return;const b=blocks[i];if(field==='start_minutes')b.start_minutes=parseTime(value);else if(field==='duration')b.duration_minutes=parseDuration(value);else if(field==='notification_enabled')b.notification_enabled=Boolean(value);else if(field==='notification_offset_minutes')b.notification_offset_minutes=Number(value)||0;else b[field]=value;if(field==='start_minutes'||field==='duration')nextBlockUpdate(i);b._dirty=true;render();const row=document.querySelector(`tr[data-id="${CSS.escape(id)}"]`);if(row){row.classList.add('updated');setTimeout(()=>row.classList.remove('updated'),700)}await saveBlock(b);if((field==='start_minutes'||field==='duration')&&i<blocks.length-1)await saveBlock(blocks[i+1])}
...
$('blocks').addEventListener('change',async e=>{const input=e.target.closest('[data-field]'),row=e.target.closest('tr');if(!input||!row)return;const field=input.dataset.field;if(field==='notification_enabled'&&input.checked)await enableNotifications();await mutateBlock(row.dataset.id,field,field==='notification_enabled'?input.checked:input.value)});
$('blocks').addEventListener('input',e=>{const input=e.target.closest('[data-field]'),row=e.target.closest('tr');if(!input||!row||['start_minutes','duration','notification_enabled','notification_offset_minutes'].includes(input.dataset.field))return;const b=findBlock(row.dataset.id);if(b)b[input.dataset.field]=input.value});
$('blocks').addEventListener('blur',async e=>{const input=e.target.closest('[data-field]'),row=e.target.closest('tr');if(!input||!row||['start_minutes','duration','notification_enabled','notification_offset_minutes'].includes(input.dataset.field))return;await mutateBlock(row.dataset.id,input.dataset.field,input.value)},true);
...
$('add-block').onclick=addBlock;$('empty-add').onclick=addBlock;
$('auth-btn').onclick=async()=>{await supabase?.auth.signOut();window.location.replace('/login.html')};
$('copy-day').onclick=()=>{const sel=$('copy-target');sel.innerHTML=DAYS.map((d,i)=>`<option value="${i}" ${i===selectedDay?'disabled':''}>${d}</option>`).join('');$('copy-source').textContent=DAYS[selectedDay];$('copy-dialog').showModal()};
$('copy-form').addEventListener('submit',e=>{e.preventDefault();copyDay();$('copy-dialog').close()});$('close-copy').onclick=()=>$('copy-dialog').close();
...
if(!supabase){window.location.replace('/login.html');}
else {
